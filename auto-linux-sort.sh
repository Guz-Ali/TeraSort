...
for num_threads in 1 2 4 8
do 
	psrecord "time sort -k 1 16gb.txt -o sort.txt --parallel=$num_threads >> linux-sort16GB.log" --log "./psrecord_analysis/linux-sort16GB-$num_threads-threads_log.txt" --interval 1 --include-children --plot "./psrecord_analysis/linux-sort16GB-$num_threads-threads_log.png"
	#time sort -k 1 16gb.txt -o sort.txt --parallel=$num_threads >> linux-sort16GB.log
	rm -rf sort.txt
done
for num_threads in 1 2 4 8
do 
	psrecord "time sort -k 1 64gb.txt -o sort.txt --parallel=$num_threads >> linux-sort64GB.log" --log "./psrecord_analysis/linux-sort64GB-$num_threads-threads_log.txt" --interval 1 --include-children --plot "./psrecord_analysis/linux-sort64GB-$num_threads-threads_log.png"
